...
export default defineConfig(({ mode }) => {
  ...
  const plugins = [
    ...
    localApiPlugin({
      ...
      worldMutationEnabled: appMode === 'local' && serverEnvironment.VITE_WORLD_MUTATION_ENABLED === 'true' && serverEnvironment.VITE_MANIFESTATION_ENABLED !== 'true',
      manifestationEnabled: appMode !== 'public' && serverEnvironment.VITE_MANIFESTATION_ENABLED === 'true',
      manifestationBenchmarkEnabled: serverEnvironment.VAYRIA_MANIFESTATION_BENCHMARK === 'true',
      manifestationRunwareEnabled: serverEnvironment.VAYRIA_MANIFESTATION_RUNWARE_ENABLED === 'true',
      manifestationBudgetLimitUsd: serverEnvironment.VAYRIA_MANIFESTATION_LATENCY_BUDGET === 'true' ? 14.825 : 10,
      manifestation: {
        expansion: serverEnvironment.VAYRIA_MANIFESTATION_EXPANSION === 'fast' ? 'fast' : 'balanced',
        transport: serverEnvironment.VAYRIA_MANIFESTATION_TRANSPORT === 'direct' ? 'direct' : 'queue',
        inputMode: serverEnvironment.VAYRIA_MANIFESTATION_INPUT === 'hosted' ? 'hosted' : 'inline',
        delivery: serverEnvironment.VAYRIA_MANIFESTATION_DELIVERY === 'stream' ? 'stream' : 'stored',
        provider: serverEnvironment.VAYRIA_MANIFESTATION_PROVIDER === 'runware' ? 'runware' : 'fal',
        mode: serverEnvironment.VAYRIA_MANIFESTATION_MODE === 'fresh-image-then-video' ? 'fresh-image-then-video' : serverEnvironment.VAYRIA_MANIFESTATION_MODE === 'reused-base-video' ? 'reused-base-video' : 'fresh-image',
        resolution: serverEnvironment.VAYRIA_MANIFESTATION_RESOLUTION === '768p' ? '768p' : '480p',
        falKey: process.env.FAL_KEY || serverEnvironment.FAL_KEY,
        runwareKey: process.env.RUNWARE_API_KEY || serverEnvironment.RUNWARE_API_KEY,
      },
