...
作品の紹介は [README](../README.md) を参照してください。
この資料はリポジトリのルートで作業する開発者・運営者向けです。コマンドとコード例のパスはリポジトリのルートを基準にします。
旧ローカル構成の手順と公開版の手順を区別してください。配信状態や実機の成功は実施時に確認します。

2026年9月23日の展示は、iPadから会場Wi-Fiで公開URLへ接続する方針です。公開版の端末登録式展示機能を使用します。配信・実機確認の成功は別途確認します。

- [展示当日のクイックスタート](exhibition-quickstart.md)
- [展示準備台帳と5候補の比較](exhibition-readiness.md)
- [電力・回線確認票](exhibition-power-network-check.md)

当日はWindows Mobile Hotspot、テザリング、モバイルルーターを代替回線にしません。PC稼働とiPad充電を含む合計100W以内を実測します。

一般公開版の検証環境、利用枠、Secretsは [一般公開版の運用手順](public-deployment.md) を参照してください。
検証URLの構成と移行手順は [検証URLの移行](staging-url-migration.md) を参照してください。
本番CDの準備、実機確認表、初回公開、自動更新、切り戻しは [本番公開手順](production-launch.md) を参照してください。本番CDは明示的に有効化するまで実行しません。
以下の準備中ページは初回公開前の構成を記録したものです。現在の配信状態を示すものではありません。本体配信後の操作は本番公開手順に従います。

## vayria.me の準備中ページ（初回公開前の参考）
...
本体の公開用Worker、認証、利用制限は実装済みです。初回公開には実機確認と公開承認が必要です。
本体を一度配信した後は、この節のplaceholder公開・rollback手順を使わず、[本番公開手順](production-launch.md)の回復用設定で戻してください。

Vayria（ヴェイリア）は、会話やカードの交換を通して、声・表情・動きで応えるAIキャラクターです。以下はローカル構成の処理を説明します。
...
以下のPython STT・Remote PCMの説明は、旧ローカル構成の`exhibition`モードを対象にします。9/23の公開版展示では [公開版の展示モード](public-exhibition.md) を参照してください。
...
[`docs/evaluation/voice-audio-lab.md`](evaluation/voice-audio-lab.md)を参照してください。
...
Presetの比較手順は[`docs/evaluation/voice-audio-lab.md`](evaluation/voice-audio-lab.md)を参照してください。
...
公開版の実装と運用は [一般公開版の運用手順](public-deployment.md)、登録・交代は [公開版の展示モード](public-exhibition.md) を参照してください。
...
以下はHotspotを利用できる環境での旧構成の参考手順です。9/23の会場では実行しません。当日は [展示クイックスタート](exhibition-quickstart.md) に従います。
...
7. 対象worktreeで、Python STTとexhibitionフロントを起動します。
   ...
   9/23当日はこの起動コマンドを使わず、[公開版の展示クイックスタート](exhibition-quickstart.md)を参照してください。
...
このローカルキャプチャは`public`モードの入力を収集しません。公開版の文字・カード・音声入力とセッション管理は [一般公開版の運用手順](public-deployment.md) を参照してください。
...
iPad Safariの実測結果は[`docs/evaluation/tts-ttfa-aivis-cloud.md`](evaluation/tts-ttfa-aivis-cloud.md)へ記録しています。
...
以下は初期ローカル版の対象範囲の記録です。現在の未実装一覧ではありません。
公開版には認証、セッション管理、SQLiteを使う利用台帳、配信設定を実装しています。
会話のセッション内保持、発話分割、割り込み処理についても初期計画後に変更があります。現在の挙動は実装と個別の検証資料を確認してください。
...
初期ローカル版は`npm run dev`と`npm run dev:exhibition`を対象としていました。
現在の公開版は [一般公開版の運用手順](public-deployment.md)、端末登録式展示は [公開版の展示モード](public-exhibition.md) を参照してください。
