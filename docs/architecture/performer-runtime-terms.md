...
| WildCard runtime design | 現在値の保持 | energy、emotion、attention、timestamps | live stateを保持する | Coreが読み、Result reducerが更新する | Performer State | 一時的なライブ状態 |
...
| WildCard runtime design | 刺激の共通化 | viewer、timer、external metadata、memory | Coreへの入力契約 | 外部イベントの後、intentの前 | Performer Trigger | 外部イベントのgeneric表現 |
...
| WildCard runtime design | 今回の実行 | preReaction、leadBeforeSpeechMs、speech、TTS、avatar | 実行可能な演技を表す | resolverの後、executorの前 | Performance Plan | 1回の演技計画 |
