...
wire上の行動名は`listen`、`backchannel`、`react_nonverbally`、`take_floor`を使います。

`listen`、`backchannel`、`react_nonverbally`では、本文とカード発動は空です。
...
GPT-Liveなどの会話相手を、Vayriaのスピーカーの近くで接続します。
各シナリオの前にセッションをリセットします。
音声の発信元は、画面の文字起こしだけで断定しません。
Vayria側の音声、アバター、タイムラインを合わせて観察します。

1. `うん`または`あー`を一度だけ話す。Vayriaは受け取り、不要な音声エコーを返さない。
2. `ねえ`または`ちょっと`と呼びかける。Vayriaは会話参加としてターンを受け取る。
3. Vayriaの発話中にタイピング音、物を置く音、周囲の短い音を入れる。音響候補が出ても、字幕、非発話モーション、会話ターンを確定前に止めない。
4. Vayriaの発話中に`待って`または`いや、今の話を止めて`と明瞭に話す。確定発話後にだけ、Vayriaが自然にターンを譲る。
5. 必要情報が不足する依頼を送る。Vayriaが具体的な質問をしたあと、回答を一つ返す。回答後に確認だけを繰り返さず、次の内容へ進む。
6. `自己紹介して`または`一つ挙げて`と依頼する。Vayriaが実際の内容を返し、行為の予告だけで止まらない。
7. 雑談または日常の依頼を送る。不要な目的、アジェンダ、担当、スケジュールを追加しない。
8. `まあ、そんな感じ`と伝える。発話が不要なら`react_nonverbally`を選び、小さく頷くだけで音声を重ねない。
9. `ここで止めよう`と伝える。Vayriaが自然な沈黙を維持し、沈黙を無理に埋めない。
10. `見に行って`など現実にできない依頼をロールプレイで送る。Vayriaが象徴的な視線や動きとして表現し、実際に確認した結果を捏造しない。

候補音のケースでは、音量低下が発生した場合も、最終文字列が空、既知誤認識、純粋な相槌なら音量が復元されることを確認します。
明瞭な内容発話だけが`confirmed_barge_in`と次の会話ターンへ進むことを確認します。
