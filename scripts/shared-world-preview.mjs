...
const storageWorker={name:'world-storage',modules:true,script:storageBundle.outputFiles[0].text,compatibilityDate:'2026-09-07',compatibilityFlags:['nodejs_compat'],bindings:{SHARED_CONVERSATION_ENABLED:'true',SHARED_HAND_ENABLED:'true'},durableObjects:{WORLD_STORAGE:{className:'WorldRoom',useSQLite:true}}};
...
await call({op:'create',roomId:'preview'});let fixtureState=await call({op:'join'});for(let i=0;i<3;i++)fixtureState=await call({op:'insert',eventId:'fixture-'+i,handCardId:fixtureState.hand[0].id,slotId:'slot-'+i,slotVersion:0,epoch:0});
const lease=await call({op:'lease',clientId:'fixture'});await call({op:'intent',clientId:'fixture',lease:lease.token,epoch:0,decisionId:'fixture-turn',intent:{actions:[{type:'prop',targetId:'',concept:'crab',sourceCardIds:fixtureState.cardSlots.flatMap(s=>s.cardId?[s.cardId]:[]),effects:['dance','float'],count:3}]}});
