...
function Resolve-ReferenceFile {
  ...
  foreach ($line in (Get-Content -LiteralPath $resolvedPath -ErrorAction Stop)) {
    ...
    if ($trimmedLine -notmatch '^(OPENAI_API_KEY|AIVIS_CLOUD_API_KEY|FAL_KEY|RUNWARE_API_KEY|GROQ_API_KEY|GEMINI_API_KEY|GOOGLE_API_KEY|DEEPSEEK_API_KEY|TYPESAFE_API_KEY)\s*=\s*(.*?)\s*$') {
