...
export function visualPrompt(intent: VisualIntent, portrait: boolean, layout?: WorldLayout) {
  ...
  return intent.type === 'background'
    ? `${STYLE} An uninhabited panoramic environment: ${subject} Scenery fills the entire image edge to edge. ${portrait ? 'Vertical' : 'Wide horizontal'} composition. The central 55 percent contains only distant scenery with soft detail. Place distinctive environmental features near the left and right edges. The bottom quarter is continuous low-contrast scenery. ${quiet ? `Keep these regions low-detail (x,y,width,height percentages): ${quiet}.` : ''} No characters, human figures, outlines of figures, interfaces, buttons, screens, frames, or lettering.`
