...
export interface WorldVisualResult {assetUrl?:string;assetSource?:WorldElement['assetSource'];error?:string;cacheOnly?:boolean}
export interface WorldExecutor {
  ...
  visual(input:{roomId:string;epoch:number;slot:ConversationSlot;element:WorldElement;mode?:'cache'|'generate'}):Promise<WorldVisualResult>;
}
...
export function executionInput(state:SharedWorldState,slot:ConversationSlot,context:string):WorldExecutionInput {
  return {roomId:state.roomId,epoch:state.epoch,slot,context,history:state.conversation?.history.slice(-20)??[],
    // A stimulus turn triggered by the newest card insert counts as a card
    // reaction, which exhibition sessions may run even while idle autonomy is off.
    reactionCardId:state.cardReaction?.id===`reaction-${state.sequence}`?state.cardReaction.cardId:undefined,
