...
export async function handle(request: Request, env: Env, ctx?: ExecutionContext, sharedContext?:string): Promise<Response> {
  ...
  if (url.pathname === '/api/admin' && request.method === 'POST') {
    ...
    if(input.op==='world-create' && typeof input.roomId==='string')return json(await createWorld(env,input.roomId,url.origin));
    if(input.op==='world-control' && typeof input.roomId==='string')return json(await worldCall(env,input.roomId,'admin','admin',{op:'control',action:input.action}));
  ...
  if(url.pathname.startsWith('/api/world-room/')){
  ...
  const attachVisual = async (response: { visualIntent?: unknown }) => {
    if(worldGuard)return { ...response, visualIntent:undefined };
  ...
  const attachWorld=async(response:object)=>{
    if(!worldGuard||!('worldIntent' in response))return;
    await worldCall(env,worldGuard.member.roomId,worldGuard.member.actor,'host',{op:'intent',...worldLease(request),decisionId:job,intent:response.worldIntent});
  };
            ...
            try {
              const response = await llmExecutionScope.run(execution, () => generate(input, false, env.OPENAI_API_KEY,
                AbortSignal.any([signal, abort.signal]), { onVisualDecision(intent) {
                  ...
                  void queue.catch(() => abort.abort());
                } }, generation, visualEnabled && !worldGuard, env.VISUAL_VIDEO_ENABLED === 'true', !!worldGuard));
              await queue;
              await attachWorld(response);
