...
export default function App() {
  ...
  useEffect(() => {
    sharedQueue.setCallbacks((text: string) => {
      ...
      void sendVoice(text, context, handleCardReplyDelivered, plan,
  ...
  }, [isPublicExhibition, sharedQueue, autonomyStateRef, setAutonomyState, interruptCurrentTurn, readCardContext, createPlanForTrigger, sendVoice, handleCardReplyDelivered, programContext, stopVoiceInput, setDucked]);
  ...
  const handleVoiceEvent = useCallback(
    (event: VoiceInputEvent) => {
      ...
      if (isPublicExhibition) {
        ...
        if (event.type === 'recognition_failed' && event.recoverable && event.segmentId) {
          playbackAtSpeechRef.current.delete(event.segmentId);
          sharedQueue.discard(true); setDucked(false);
          return;
        }
        if (event.type === 'recognition_stopped' || event.type === 'recognition_failed' ||
            (event.type === 'listening_pending' && event.reason === 'capture-starting')) {
          sharedQueue.reset(); sharedGenerationRef.current++; playbackAtSpeechRef.current.clear();
