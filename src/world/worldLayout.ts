...
export interface PropGeometry { bounds: WorldRect; pivot: WorldPoint; aspect: number; displayWidth: number; maxWidth?: number; shrinkSteps?: readonly number[] }
export function placeWorldProp(layout: WorldLayout, anchor: WorldPoint, geometry: PropGeometry, scale = 1, occupied: WorldRect[] = []): { rect: WorldRect; visible: WorldRect } | null {
  ...
  for (const shrink of geometry.shrinkSteps ?? [1, .8, .6, .45]) {
