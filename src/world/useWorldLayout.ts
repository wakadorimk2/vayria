...
export function useWorldLayout(root: RefObject<HTMLDivElement | null>, stage: RefObject<VrmStageHandle | null>, runtime: WorldRuntime) {
  ...
  useEffect(() => {
    const sample = () => {
      ...
      const obstacles = [...document.querySelectorAll<HTMLElement>('.card-zone, .conversation-copy, .subtitle, .subtitle-overlay, .speech-caption, .performer-caption, .public-controls__actions')].flatMap(node => {
