...
export interface SharedWorldState {
  ...
  physics?: { mode:'normal'|'water'|'zero'; effects:WorldEffect[] };
  desiredBackgroundId?:string;
}
...
export function applyWorldIntent(state:SharedWorldState,intent:WorldIntent,decisionId:string,now:number){
  ...
  for(const a of intent.actions){if(a.sourceCardIds.some(id=>!state.weights[id]))throw new WorldError('unknown_card_source',400);
    ...
    if((a.type==='effect'||a.type==='duplicate'||(a.type==='physics'&&a.targetId!==''))&&!state.elements.some(e=>e.id===a.targetId&&e.status==='displayed'))throw new WorldError('unknown_world_target',400);}
  const created:WorldElement[]=[];
  for(const [i,a]of intent.actions.entries()){
    if(a.type==='physics'){
      if(a.targetId){const target=state.elements.find(e=>e.id===a.targetId&&e.status==='displayed');if(!target)throw new WorldError('unknown_world_target',400);target.effects=[...new Set([...target.effects.filter(e=>a.concept!=='normal'||e!=='float'),...a.effects,...(a.concept!=='normal'?['float' as const]:[])])];target.sourceCardIds=[...new Set([...target.sourceCardIds,...a.sourceCardIds])];target.reinforcedAt=now;}
      else state.physics={mode:a.concept==='water'?'water':a.concept==='zero'?'zero':'normal',effects:a.effects};
      continue;
    }
    ...
    if(same){if(same.kind==='background')state.desiredBackgroundId=same.id;same.count=Math.min(1000,same.count+count);same.effects=[...new Set([...same.effects,...a.effects])];same.reinforcedAt=now;same.sourceCardIds=[...new Set([...same.sourceCardIds,...a.sourceCardIds])];continue;}
    ...
    state.elements.push(element);created.push(element);if(element.kind==='background')state.desiredBackgroundId=element.id;
  }
