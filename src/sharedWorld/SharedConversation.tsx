...
export function SharedConversation({snapshot,muted,play,stop,onEmotion,onMotion}:{snapshot:WorldSnapshot|null;muted:boolean;play:PlayAudio;stop:()=>void;onEmotion:(emotion:Emotion)=>void;onMotion:(asset:string,id:number)=>void}){
  const seen=useRef(new Set<string>());const current=useRef({muted,play,stop,onEmotion,onMotion});
  ...
  const replyKey=reply?`${snapshot?.epoch}:${reply.id}`:'';
  useEffect(()=>{
    const {reply,epoch,offset}=payload.current;
    if(!reply||epoch===undefined)return;
    const key=`${epoch}:${reply.id}`;if(seen.current.has(key))return;
    ...
    const timer=setTimeout(()=>{seen.current.add(key);if(seen.current.size>128)seen.current.delete(seen.current.values().next().value!);current.current.onEmotion(normalizeEmotion(reply.emotion));
      ...
      void(async()=>{try{const source=await audio;if(!source||controller.signal.aborted||current.current.muted)return;started=true;await current.current.play(source);}catch{/* Captions remain available when playback is unavailable. */}})();
    },Math.max(0,reply.startsAt-Date.now()-offset));
    return()=>{controller.abort();clearTimeout(timer);if(started)current.current.stop();};
