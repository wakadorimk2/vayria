...
export function createCloudVoiceAdapter(options: VoiceInputAdapterOptions): VoiceInputAdapter {
  ...
  const flush = async () => {
    ...
    busy = true; emit({ type: 'speech_ended', segmentId: id, at: Date.now() });
    try {
      ...
      const response = await publicFetch('/api/transcribe', { method: 'POST', headers: { 'Content-Type': 'audio/wav' }, body: buffer });
      if (!response.ok) throw new Error('transcription failed');
      const result = await response.json();
      if (enabled && generation === current) emit({ type: 'utterance_finalized', segmentId: id, text: result.text, at: Date.now() });
    } catch { if (enabled && generation === current) emit({ type: 'recognition_failed', code: 'recognition-failed', at: Date.now() }); }
    finally { busy = false; }
  };
  const stop = async () => {
    enabled = false; generation++; reset(); node?.disconnect(); node = null;
    media?.getTracks().forEach(track => track.stop()); media = null;
    const previous = context; context = null; if (previous && previous.state !== 'closed') await previous.close();
  ...
  return {
    ...
    async start() {
      ...
      try {
        ...
        if (current !== generation || !publicActive()) { await stop(); return false; }
        ...
        context.createMediaStreamSource(media).connect(node); node.connect(context.destination); await context.resume(); enabled = true;
      ...
      } catch { await stop(); emit({ type: 'recognition_failed', code: 'audio-capture', at: Date.now() }); return false; }
