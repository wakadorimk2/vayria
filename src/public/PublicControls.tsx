...
export default function PublicControls({ generation, settingsLayout, onSettingsOpenChange, cardsOpen, textOpen, onCardsToggle, greetingComplete, greetingBusy, onGreeting, themePreference, resolvedTheme, onThemeChange, isMuted, onMuteToggle, microphoneOn, microphoneState, microphoneLevel, microphoneNotice, onMicrophoneToggle }: {
  ...
  return <aside className="public-controls" aria-label="会話の操作">
    ...
    <div className="public-controls__actions">
      ...
      {generation && <button className="public-controls__generation" aria-label={`生成モード ${generation.enabled ? 'ON' : 'OFF'}`} title="生成モード" aria-pressed={generation.enabled} disabled={generation.busy} onClick={() => { void generation.toggle().then(() => setGenerationNotice(true)); }}><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m9 3 2.1 5.9L17 11l-5.9 2.1L9 19l-2.1-5.9L1 11l5.9-2.1Z"/><path d="m19 2 1 3 3 1-3 1-1 3-1-3-3-1 3-1Z"/></svg></button>}
