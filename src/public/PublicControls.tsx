...
import { activatePublic, cancelPublicAction, pausePublic, publicActive, publicSessionId, registerPublicSessionRequest, subscribePublic, type PublicStatus } from './session';
...
export default function PublicControls({ cardsOpen, textOpen, onCardsToggle, greetingComplete, greetingBusy, onGreeting, themePreference, resolvedTheme, onThemeChange, isMuted, onMuteToggle, microphoneOn, microphoneState, microphoneLevel, onMicrophoneToggle }: {
  ...
  useEffect(() => {
    if (!noticeOpen || active || !status?.enabled || !status.siteKey || !challenge.current) return;
    let disposed = false;
  ...
  }, [status?.siteKey, status?.enabled, noticeOpen, active, finishRequest]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setStatus(s => ({ ...s, ...value })); activatePublic(value.session); finishRequest(true); setNoticeOpen(false);
  ...
  return <aside className="public-controls" aria-label="会話の操作">
    ...
    <div className="public-controls__actions">
      ...
      <button className="public-controls__disclosure" aria-label="設定" title="設定" aria-expanded={expanded} aria-controls="public-session-panel" onClick={() => { if (expanded) cancelRequest(); else if (panelRef.current) panelRef.current.scrollTop = 0; setExpanded(value => !value); }}>
    ...
    <h3>利用状況</h3>
    <p aria-live="polite">{active ? '会話中です。' : status?.enabled === false || status?.stopped ? '現在は会話を休止しています。' : '会話は開始していません。'} {status && `残り: 本日${status.remainingDay}回・今月${status.remainingMonth}回`}</p>
