...
import { activatePublic, pausePublic, publicActive, publicSessionId, registerPublicSessionRequest, subscribePublic, updatePublicStatus, type PublicStatus } from './session';
...
export default function PublicControls({ themePreference, resolvedTheme, onThemeChange, isMuted, onMuteToggle, microphoneOn, microphoneState, microphoneLevel, onMicrophoneToggle }: {
  ...
  const exhibitionId = status?.exhibition?.id;
  ...
  useEffect(() => {
    if (!expanded || active || !status?.enabled || status.exhibition || !status.siteKey || !challenge.current) return;
    let disposed = false;
  ...
  }, [status?.siteKey, status?.enabled, status?.exhibition, expanded, active, finishRequest]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setStatus(s => ({ ...s, ...value })); updatePublicStatus(value); setStatusStale(false); activatePublic(value.session); finishRequest(true); setExpanded(false);
  ...
  return <aside className="public-controls" aria-label="会話の操作">
    {status?.exhibition && <div className="public-exhibition-handoff">
      <button onClick={() => { cancelRequest(); window.dispatchEvent(new Event('vayria-exhibition-next')); }}>体験を終える</button>
      {(!status.exhibition.available || status.stopped || !status.enabled) && <span className="public-exhibition-paused" role="status">展示を休止しています。設定を確認してください。</span>}
    </div>}
    <div className="public-controls__actions">
      ...
      <button className="public-controls__disclosure" aria-label={status?.exhibition?.warning ? '設定：展示予算の通知あり' : '設定'} title="設定" aria-expanded={expanded} aria-controls="public-session-panel" onClick={() => { automaticMessageRef.current = false; if (expanded) cancelRequest(); else if (panelRef.current) panelRef.current.scrollTop = 0; setExpanded(value => !value); }}>
    ...
    <h3>利用状況</h3>
    <p ref={messageRef} aria-live="polite">{message} {status && !status.exhibition && `残り: 本日${status.remainingDay}回・今月${status.remainingMonth}回`}</p>
